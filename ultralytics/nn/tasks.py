...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
           ...
           KWConv, DySnakeConv,
            OREPA, OREPA_LargeConv, RepVGGBlock_OREPA, RFAConv, RFCAConv, RFCBAMConv, C2f_HFERB
        }:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
        ...
        if isinstance(c2, list) and m not in {ChannelTransformer, CrossLayerChannelAttention, CrossLayerSpatialAttention, MutilScaleEdgeInfoGenetator}:
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
